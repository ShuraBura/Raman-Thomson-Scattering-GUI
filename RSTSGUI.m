...
% =========================================================================
%                COMPREHENSIVE RAMAN & THOMSON ANALYSIS (V9.8 GUI)
% =========================================================================
% V9.8 Changes:
...
update_images_and_status(handles);
    ...
    function intensity_calibration_callback(hObject, ~)
        ...
        for start_idx = 1:num_starts
            ...
            try
                [p_fit_current, resnorm_current, ~, exitflag, ~, ~, jacobian_current] = lsqcurvefit(shape_objective, p0_current, wavelength_nm(fit_mask), thomson_spectrum(fit_mask), lb, ub, options);
        ...
        disp('--- Intensity Calibration: Stage 2 - Calculate ne and errors ---');
        ...
        if dof > 0 && ~isempty(best_jacobian) && rcond(full(best_jacobian' * best_jacobian)) > 1e-15
            ...
            Te_fit_err = p_err(1);
            amp_err = p_err(3);
            % Since ne is proportional to total counts, which is proportional to amplitude:
